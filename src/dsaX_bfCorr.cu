...
void dbeamformer(dmem * d) {
  ...
  begin = clock();
  cudaMemcpy(d->d_input,d->h_input,NPACKETS_PER_BLOCK*NANTS*NCHAN_PER_PACKET*4,cudaMemcpyHostToDevice);
  end = clock();
  d->cp += (float)(end - begin) / CLOCKS_PER_SEC;
  ...
  sum_ib<<<NCHAN_PER_PACKET*2*NPACKETS_PER_BLOCK,32>>>(d->d_bar,d->d_bai,d->d_bbr,d->d_bbi,d->d_ibsum,d->d_flagants);
  ...
  sum_transpose_and_scale_output<<<dimGrid,dimBlock>>>(d->d_htx,d->d_bigpower,d->subtract_ib);
...
__global__ void populate_weights_matrix(float * antpos_e, float * antpos_n, float * calibs, half * war, half * wai, half * wbr, half * wbi, float * fqs, float dec) {
  ...
  // temp weights
  war[inidx] = __float2half(1.0);                                                                                                 
  wai[inidx] = __float2half(0.0);
  wbr[inidx] = __float2half(1.0);                                                                                                 
  wbi[inidx] = __float2half(0.0);
  
  /* This was for DSA-110 - will need to be rewritten sometime for CASM
...
void calc_weights(dmem * d) {
  ...
  populate_weights_matrix<<<2*(NCHAN_PER_PACKET/8)*(NBEAMS)*(NANTS)/128,128>>>(d_antpos_e,d_antpos_n,d_calibs,d->weights_a_r,d->weights_a_i,d->weights_b_r,d->weights_b_i,d->d_freqs,OVRO_LAT-(d->obsdec));  
...
int main (int argc, char *argv[]) {  
  ...
  // replace with device from command line
  //  cudaSetDevice(1);
  ...
  if (bf) {
    ...
    if (!(ff=fopen(fflagants,"r"))) {
      syslog(LOG_ERR,"could not open flagants file - using default settings\n");
    }
    else {
      while (!feof(ff)) {
	fscanf(ff,"%d\n",&d.flagants[iii]);
    ...
    if (!(ff=fopen(fcalib,"rb"))) {
      syslog(LOG_ERR,"could not open calibss file - using default settings\n");
    ...
    for (iii=0;iii<(NCHAN_PER_PACKET/8);iii++)
      d.h_freqs[iii] = 1e6*(sfreq-iii*BEAM_CHBW_MHZ);
